...
def create_students(school, count=15):
    """Create only the missing number of test students for a school."""
    try:
        ...
        existing_count = Student.objects.filter(school=school).count()
        missing_count = max(count - existing_count, 0)
        if not missing_count:
            log(f"  Student data already complete ({existing_count} students)", 'WARN')
            return
...
def create_staff_profiles(school, teachers, subjects):
    ...
    try:
        ...
        for i, user in enumerate(teachers):
            ...
            # Keep every seeded timetable entry valid when the test data is
            # re-run after timetable assignments have changed.
            if subjects:
...
def create_timetable_data(school):
    ...
    try:
        ...
        teacher_subject_ids = {
            profile.user_id: set(profile.subjects.filter(school=school).values_list('pk', flat=True))
            for profile in StaffProfile.objects.filter(
                school=school, user__in=teachers, staff_type='teacher', is_active=True,
        ...
        for cls_idx, classroom in enumerate(classrooms[:3]):  # seed first 3 classrooms
            ...
            for day_idx, day in enumerate(days):
                for slot_idx, slot in enumerate(course_slots[:4]):  # 4 course slots per day
                    ...
                    eligible_teachers = [
                        candidate for candidate in teachers
                        if subject.pk in teacher_subject_ids.get(candidate.pk, set())
                    ]
                    if not eligible_teachers:
                        conflicts_skipped += 1
                        continue
                    teacher = eligible_teachers[(cls_idx + day_idx + slot_idx) % len(eligible_teachers)]
                    ...
                    if TimetableEntry.objects.filter(school=school, teacher=teacher, day=day, timeslot=slot).exclude(classroom=classroom).exists():
                        # Try next teacher
                        for alt_teacher in eligible_teachers:
